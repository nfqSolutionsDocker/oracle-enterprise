...
export DATABASE=$1
alert_log=/u01/oracle/app/diag/rdbms/$(echo $DATABASE | tr [:upper:] [:lower:])/$DATABASE/trace/alert_$DATABASE.log
...
echo "DATAFILEDESTINATION=/u01/oracle/app/oradata" >> /u01/initial/dbca_$DATABASE.rsp
echo "RECOVERYAREADESTINATION=/u01/oracle/app/oradata" >> /u01/initial/dbca_$DATABASE.rsp
...
echo "$DATABASE.__oracle_base='/u01/oracle/app'#ORACLE_BASE set from environment" >> /u01/initial/spfile_$DATABASE.ora
...
echo "*.audit_file_dest='/u01/oracle/app/admin/$DATABASE/adump'" >> /u01/initial/spfile_$DATABASE.ora
echo "*.audit_trail='db'" >> /u01/initial/spfile_$DATABASE.ora
echo "*.compatible='11.2.0.0.0'" >> /u01/initial/spfile_$DATABASE.ora
echo "*.control_files='/u01/oracle/app/oradata/$DATABASE/control01.ctl','/u01/oracle/app/flash_recovery_area/$DATABASE/control02.ctl'" >> /u01/initial/spfile_$DATABASE.ora
...
echo "*.db_recovery_file_dest='/u01/oracle/app/flash_recovery_area'" >> /u01/initial/spfile_$DATABASE.ora
...
echo "*.diagnostic_dest='/u01/oracle/app'" >> /u01/initial/spfile_$DATABASE.ora
...
echo "if [ ! -f /u01/oracle/app/product/11.2.0/dbhome_1/dbs/init$DATABASE.ora ]; then" >> /u01/initial/create_databases.sh
...
echo "source /u01/initial/colorecho" >> /u01/initial/startup_$DATABASE.sh
echo "source ~/.bashrc" >> /u01/initial/startup_$DATABASE.sh
echo "" >> /u01/initial/startup_$DATABASE.sh
